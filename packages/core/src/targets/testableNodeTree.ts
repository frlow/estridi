...
export type Block = {
  touched: string[]
  nodes: (ScrapedNode | Block)[]
  name: string
  blockPath: ScrapedNode[]
}
export const getTestableNodeTree = (scraped: Scraped) => {
  ...
  while (toProcess.length > 0) {
    ...
    toProcess.push(
      ...connections.map((c) => {
        const connectionNode = scraped.find((n) => n.id === c)
        return { ...connectionNode, block: current.block }
      }),
    )
    if ('link' in current && current.link) {
      ...
      const newBlock = {
        ...
        nodes: [],
        name: linked.text,
        blockPath: findShortestPathToNode(
          scraped,
          current.id,
          current.block.blockPath,
        ),
      }
  ...
  function removeTouched(obj) {
    for (const prop in obj) {
      if (prop === 'touched') delete obj[prop]
      else if (typeof obj[prop] === 'object') removeTouched(obj[prop])
    }
  }
