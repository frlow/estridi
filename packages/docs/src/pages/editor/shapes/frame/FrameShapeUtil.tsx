import {
  ...
  type TLFrameShape as TLBaseFrameShape,
  type TLFrameShapeProps as TLBaseFrameShapeProps,
  type TLGroupShape,
...
import { FrameHeading } from './components/FrameHeading.tsx'
import { FrameColorPicker } from './components/FrameColorPicker.tsx'
import {
  getFrameHeadingOpts,
  getFrameHeadingSide,
  getFrameHeadingSize,
  getFrameHeadingTranslation,
} from './frameHelpers.ts'
import { FRAME_COLOR_TRANSPARENT } from '../util/constants.ts'
...
export class FrameShapeUtil extends BaseBoxShapeUtil<TLBaseFrameShape> {
  ...
  override component(shape: TLBaseFrameShape) {
    ...
    const isCreating = useValue(
      ...
      () => {
        ...
        const info = (
          resizingState as typeof resizingState & {
            info: { isCreating: boolean }
          }
        )?.info
        if (!info) return false
        return (
          info.isCreating && this.editor.getOnlySelectedShapeId() === shape.id
        )
      },
      [shape.id],
    )
    ...
    const color =
      ((shape.props as CustomFrameShapeProps)
        .color as keyof typeof FRAME_COLOR_TRANSPARENT) || 'blue'

    return (
      <HTMLContainer>
        <SVGContainer>
          <rect
            className={classNames('tl-frame__body', {
  ...
  override toSvg(shape: TLBaseFrameShape, ctx: SvgExportContext) {
    ...
    const opts: TLCreateTextJsxFromSpansOpts = getFrameHeadingOpts(
      shape,
      theme.text,
    )
