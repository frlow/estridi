import { createFeature, Feature, ParsedNode, ParsedTable, prepareNode } from './figma/feature'
...
figma.ui.onmessage = (msg) => {
  if (msg.type === 'traverse') {
    ...
    for (const section of sections as SectionNode[]) {
      ...
      const handledNodes = startNodes.map(s => prepareNode(traverse(s))).filter(s => !!s).flatMap(s => s as ParsedNode[][])
