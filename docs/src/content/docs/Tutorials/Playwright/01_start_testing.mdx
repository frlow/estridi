...
```typescript
    test_demoDataJson: async ({ page }) => {
      const requestPromise = page.waitForRequest('/demo/data.json')
      return async () => {
        const request = await requestPromise
        expect(await request.headerValue('x-my-data')).toEqual('dummy')
      }
    }
```
...
```typescript
   type Country = {
    ...
    useEffect(() => {
        fetch('/demo/data.json', {headers: {'x-my-data': 'something'}})
            .then((res) => {
...
Then in our public directory create a new folder called `demo` and in that folder create a file called `data.json`.
...
First make sure your "mock" works by going to `http://localhost:XXXX/demo/data.json` and you should see the json file.

`http://localhost:5173/demo/data.json`
<div data-lightbox="true">
...
test_demoDataJson: async ({ page }) => {
  ...
  return async () => {
    const request = await requestPromise
    expect(await request.headerValue('x-my-data')).toEqual('dummy') // This is "something" in the example
  }
}
```

And this is **good** because as mentioned before we want to invert the test and see that it fails. So let's fix the app by changing the sent header to "dummy".
<div data-lightbox="true">
